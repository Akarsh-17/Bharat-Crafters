import React from 'react'
import IconBtn from './IconBtn'

const ConfirmationModal = ({modal}) => {
  return (
    <div className='fixed inset-0 z-[1000] !mt-0 grid place-items-center overflow-auto bg-white bg-opacity-10 backdrop-blur-sm'>
        <div className='flex flex-col w-11/12 max-w-[350px] rounded-lg border border-richblack-400 bg-richblack-800 p-6'>
            <p className='text-2xl font-semibold text-richblack-5'>{modal?.text1}</p>
            <p className="mt-3 mb-5 leading-6 text-richblack-200">{modal?.text2}</p>
            <div className='flex items-center gap-x-4'>
                <IconBtn
                  text={modal?.btnText1}
                  onclick={modal?.btn1Handler}
                />
                <button
                 onClick={modal?.btn2Handler}
                 className='cursor-pointer rounded-md  bg-richblack-200 py-2 px-5 font-semibold text-richblack-900 '
                >
                    {modal?.btnText2}
                </button>
            </div>
        </div>
    </div>
  )
}

export default ConfirmationModal